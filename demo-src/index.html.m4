...
<p><tt>GNU Make</tt> is a tool that figures out, from your collection of source files, which commands need to be run to render those files into HTML. When invoked multiple times it is smart enough to only render those files whose source files have changed. Modern work-alikes include <a href="gruntjs.com">Grunt</a>, <a href="http://rake.rubyforge.org">Rake</a>, or <a href="http://ant.apache.org">Ant</a>.</p>
